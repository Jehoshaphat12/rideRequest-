...
export default function RiderDeliveryHistoryScreen() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const totalEarnings = completedDeliveries.reduce((sum, delivery) => sum + delivery.estimatedFare, 0);
  ...
  const renderDeliveryItem = ({ item }: { item: DeliveryHistoryItem }) => (
    <TouchableOpacity
      ...
      onPress={() => router.push(`/(rider)/deliveryProgress?deliveryId=${item.id}`)}
    >
        ...
        <View style={styles.statusContainer}>
          ...
          {item.urgent && (
            <View style={[styles.urgentBadge, { backgroundColor: theme.warning }]}>
              <Ionicons name="flash" size={10} color={theme.primaryText} />
            </View>
          )}
        </View>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: theme.card }]}>
        ...
        <Text style={[styles.headerTitle, { color: theme.text }]}>My Deliveries</Text>
